# ===============================
# train_kfold_text_plus_scale_M1_pure.py
# M1 pure: 病历文本 + 4个量表总分 的联合模型
#
# 统一版本：
...
# -----------------------
# 2) Custom model: BERT + scale totals
# -----------------------
class BertWithScaleClassifier(PreTrainedModel):
    config_class = AutoConfig
    base_model_prefix = "text_encoder"

    def __init__(self, config, num_scale_features=4):
        ...
        self.classifier = nn.Linear(config.hidden_size + num_scale_features, config.num_labels)

        self.config.num_scale_features = num_scale_features
        self.post_init()

    @classmethod
    def from_pretrained_with_scales(cls, model_name_or_path, num_labels=2, num_scale_features=4):
        config = AutoConfig.from_pretrained(model_name_or_path, num_labels=num_labels)
        model = cls(config, num_scale_features=num_scale_features)
        pretrained_encoder = AutoModel.from_pretrained(model_name_or_path, config=config)
        model.text_encoder = pretrained_encoder
        model.config.num_scale_features = num_scale_features
        return model
    ...
    def forward(
        ...
        **kwargs
        ...
        if scale_features is None:
            ...
            scale_features = scale_features.to(text_repr.device).float()

        fused = torch.cat([text_repr, scale_features], dim=1)
...
# -----------------------
# 3) Data collator: text padding + numeric features
# -----------------------
class TextScaleDataCollator:
    ...
    def __call__(self, features):
        ...
        batch["scale_features"] = torch.tensor(
            [f["scale_features"] for f in features],
            dtype=torch.float
        )
...
def main(
    ...
    exp_name="text_plus_scale_kfold_M1_pure",
    model_ckpt="bert-base-chinese",
    seed=42,
    inner_val_ratio=0.2,   # 保留参数仅为兼容；本脚本不再重新split
    num_train_epochs=6,
    batch_size=8,
    max_length=512,
    threshold_mode="f1",
    ...
    required_cols = ["ID", "history_text", "label", "fold"] + scale_cols
    missing = [c for c in required_cols if c not in df.columns]
    if missing:
        raise ValueError(f"Missing columns: {missing}")

    df = df[required_cols].copy()
    df.columns = ["id", "text", "label", "fold"] + scale_cols
    ...
    # ---- 不允许静默丢样本：发现问题直接报错
    bad_empty_text = df["text"].str.strip() == ""
    if bad_empty_text.any():
        bad_ids = df.loc[bad_empty_text, "id"].astype(str).tolist()[:10]
        raise ValueError(f"Empty text found in base file. Example IDs: {bad_ids}")
    ...
    data_collator = TextScaleDataCollator(tokenizer=tokenizer)
    ...
    for fold in folds:
        ...
        def build_model_df(xdf):
            out = xdf[["id", "text", "label"]].copy()
            out["scale_features"] = xdf[scale_cols].values.tolist()
            return out

        train_model_df = build_model_df(df_train)
        val_model_df = build_model_df(df_val)
        test_model_df = build_model_df(df_test)

        # ---- datasets
        train_ds = Dataset.from_pandas(train_model_df, preserve_index=False)
        val_ds = Dataset.from_pandas(val_model_df, preserve_index=False)
        test_ds = Dataset.from_pandas(test_model_df, preserve_index=False)
        ...
        # ---- model
        model = BertWithScaleClassifier.from_pretrained_with_scales(
            model_ckpt,
            num_labels=2,
            num_scale_features=len(scale_cols)
        ).to(device)
