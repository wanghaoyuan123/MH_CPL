# ===============================
# train_kfold_text_only_M5_pure.py
# 基于 M2_pure 改造：
# 1) 输入文本列改为 text_M5
...
# -----------------------
def main(
    data_file=os.path.join(BASE_DIR, "ML_dataset_balanced_5fold_text_M5_pure_depression.xlsx"),
    exp_root=os.path.join(BASE_DIR, "experiments"),
    exp_name="text_only_kfold_M5_pure",
    model_ckpt="bert-base-chinese",
    seed=42,
    inner_val_ratio=0.2,   # 保留参数仅为兼容；本脚本不再重新 split
    num_train_epochs=6,
    batch_size=8,
    max_length=512,
    threshold_mode="f1",
    ...
    required_cols = ["ID", text_col, "label", "fold"]
    ...
    for fold in folds:
        ...
        split_df.to_csv(
            os.path.join(fold_dir, "splits.csv"),
            index=False,
            encoding="utf-8-sig"
        )
        ...
        train_ds = Dataset.from_pandas(
            df_train[["id", "text", "label"]],
            preserve_index=False
        )
        val_ds = Dataset.from_pandas(
            df_val[["id", "text", "label"]],
            preserve_index=False
        )
        test_ds = Dataset.from_pandas(
            df_test[["id", "text", "label"]],
            preserve_index=False
        )
        ...
        if not df_summary.empty:
            ...
            plt.plot(
                df_summary["Epoch"],
                df_summary["Training Loss"],
                label="Training Loss"
            )
            plt.plot(
                df_summary["Epoch"],
                df_summary["Validation Loss"],
            ...
            if "Val F1_pos1@0.5" in df_summary.columns:
                plt.plot(
                    df_summary["Epoch"],
                    df_summary["Val F1_pos1@0.5"],
                    label="Val F1_pos1@0.5"
                )

            if "Val Recall_pos1@0.5" in df_summary.columns:
                plt.plot(
                    df_summary["Epoch"],
                    df_summary["Val Recall_pos1@0.5"],
                    label="Val Recall_pos1@0.5"
                )

            if "Val Precision_pos1@0.5" in df_summary.columns:
                plt.plot(
                    df_summary["Epoch"],
                    df_summary["Val Precision_pos1@0.5"],
                    label="Val Precision_pos1@0.5"
                )
            ...
            plt.legend()
            plt.savefig(
                os.path.join(fold_dir, "val_metrics_curve.png"),
                dpi=300,
                bbox_inches="tight"
            )
            plt.close()
        ...
        if threshold_mode == "recall":
            ...
            best_t, best_t_f1 = select_threshold_by_f1_pos1(
                y_val,
                p1_val
            )
        ...
        print(
            f"[Fold {fold}] VAL AUC={val_auc:.3f}  "
            f"pred_pos_rate@0.5={val_pred_pos_rate_05:.3f}"
        )
        print(
            f"[Fold {fold}] Best threshold from VAL = {best_t:.2f} "
            f"(val_f1_pos1={best_t_f1}) "
            f"pred_pos_rate@best_t={val_pred_pos_rate_bt:.3f}"
        )
        ...
        cm_df.to_csv(
            os.path.join(fold_dir, "confusion_matrix.csv"),
            encoding="utf-8-sig"
        )
    ...
    cv_df.to_csv(
        os.path.join(exp_dir, "cv_metrics_by_fold.csv"),
        index=False,
        encoding="utf-8-sig"
    )
    ...
    metric_cols = [
        ...
        "test_precision_pos1",
        "test_recall_pos1",
        "test_pred_pos_rate@0.5",
        "test_pred_pos_rate@best_t"
    ]
    ...
    summary_df.to_csv(
        os.path.join(exp_dir, "cv_summary_mean_std.csv"),
        index=False,
        encoding="utf-8-sig"
    )
    ...
    oof_all.to_csv(
        os.path.join(exp_dir, "oof_predictions_all.csv"),
        index=False,
        encoding="utf-8-sig"
    )
    ...
    diag_df.to_csv(
        os.path.join(exp_dir, "diagnostics_prob_summary.csv"),
        index=False,
        encoding="utf-8-sig"
    )
